...
if (isset($_REQUEST['email']) || isset($_REQUEST['password']))
{
    ...
    # SERVER QUERY
    $sign_in_query = 'SELECT * FROM celteck_user WHERE user_email_address ="'. $email.'"';
...
# New Account
if (isset($_REQUEST['fullName']) || isset($_REQUEST['addEmail']))
{
    // Sanitize Name and Email
    $email = filter_var(trim($_REQUEST['addEmail']), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z0-9.@_-]+$/")));
    $name = filter_var(trim($_REQUEST['fullName']), FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")));
    if ($email == NULL)
    {
        print('<small class="alert alert-warning w3-animate-bottom">Invalid Email</small>');
        return FALSE;
    }
    elseif ($name == NULL)
    {
        print('<small class="alert alert-warning w3-animate-bottom">Invalid Name</small>');
        return FALSE;
    }

    if ($_REQUEST['addPassword'] == $_REQUEST['confirmPassword'])
    {
        $sql_query = 'INSERT INTO celteck_user (user_full_name, user_email_address, user_password, user_security_question, user_answer, date_reg)
            VALUES ("'.$name.'","'.$email.'","'.password_hash($_REQUEST["addPassword"], PASSWORD_DEFAULT).
    ...
    {
        print('<small class="alert alert-danger w3-animate-bottom">Passwords does not match</small>');
        return FALSE;
    }
}
